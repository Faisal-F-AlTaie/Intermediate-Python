...
# input the height,colums, and row inputs (all as integer)
height = int(input("height? :"))
colomns = int(input("colomns? :"))
rows = int(input("row? :"))

# show what thr triangles would br formed of and showing our height list near the top
character = '*'
heightList = []

# this is the for loop (take the range from 1 to height multiplied by 2 and increase by 2 repeaditly)
for h in range(1, height * 2, 2):
    heightList.append(h)

# his is the for loop that takes the range on the row and make the row -1
for row in range(rows):
    for num in heightList:
        ns = ''

        # if the row is diviv=sble by 2 then its 0 and if its not divisble by 2 it will turn to 1
        if row % 2 == 0:
            number = character * num

            # for each colomn it will itterate over from 0 to int(colomns)
            for colomn in range(colomns):
                ns += ' ' + number.center(max(heightList))

            print(ns)

        # elif the row is diviv=sble by 2 then its 0 and if its not divisble by 2 it will turn to 1
        elif row % 2 == 1:
            ns = ''
            number = character * num
            ...
            for colomn in range(colomns):
                ...
                # creating the last off set triangle
                if colomn + 1 == colomns:
